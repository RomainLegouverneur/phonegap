...
// Langues
var HYPERCUBE_EXCELLENT = 'EXCELLENT!';
var GREAT = 'BIEN JOUE!'; 
var MOVE = 'mouvement';
var MOVES = 'mouvements';
var GAME_OVER = 'Perdu !';
var POINTS = 'points';
var PLAY_AGAIN = 'Recommence!';
var RATE_ME = 'vote pour moi!';
var AUTHOR = 'Auteur'; 
var MULTIPLIER_COMBO = 'SUPER COMBO!';
var MULTIPLIER_HYPERCUBE = 'HYPERCUBE!';
var MULTIPLIER_FIREBALL = 'FIREBALL!';
var BEST_SCORE = 'Meilleur score';
var BEST_COMBO = 'Meileur combo';
...
$(function () { // DOM ready
  ...
  if($('#android_link').length) {
    ...
    if(ua.indexOf('android') > -1) {
      $('#android_link').attr('href', 'market://details?id=com.phonegap.test');
    }
...
function verif_tableau() {
  ...
  if ($icone != undefined && $binome != undefined) {
    if ($icone.hasClass('hypercube') && !$icone.hasClass('new')) {
      ...
      $('#zone_message').append('<div class="hypercube">'+ HYPERCUBE_EXCELLENT +'</div>');
  ...
  if (chaine_trouvee) {
    ...
    if ($icone == undefined && $binome == undefined) {
      if (test_possible_move()) {
        ...
        $('#zone_message').html('<div class="bad">'+ GAME_OVER +'</div>'); //Game over
        $('#zone_message').append('<div class="good">' + score + ' ' + POINTS + '</div>'); //Points
        if (score > best_score) {
          best_score = score;
        ...
        //$('#zone_message').append('<a target="_blank" href="http://card.xstyled.net" class="button">'+ RATE_ME +'</a>'); //Rate Me
		//$('#zone_message').append('<a target="_blank" href="http://www.xstyled.net" class="button">'+ AUTHOR +'</a>'); //Author
        $('#zone_message').append('<div class="button bt_new_game">'+ PLAY_AGAIN +'</div>'); //Play again!
    ...
    if (suite_verti >= 3) {
      ...
      while(ligne+i < NB_COLONNES && tab_icones[ligne+i][colonne] == num_icone) {
        ...
        if(i == 1 && multiplier%5 == 0){
          ...
          $('#zone_message').append('<div class="star">'+ MULTIPLIER_COMBO +'</div>'); //SUPER COMBO!
        } else {
          if(i == 1 && suite_verti == 4){
            ...
            $('#zone_message').append('<div class="fire">'+ MULTIPLIER_FIREBALL +'</div>'); //FIREBALL!
          } else {
            if(i == 2 && suite_verti == 5){
              ...
              $('#zone_message').append('<div class="hypercube">'+ MULTIPLIER_HYPERCUBE +'</div>'); //HYPERCUBE!
            } else {
    ...
    if (suite_horiz >= 3) {
      ...
      while(colonne+i < NB_LIGNES && tab_icones[ligne][colonne+i] == num_icone) {
        ...
        if(i == 1 && multiplier%5 == 0){
          ...
          $('#zone_message').append('<div class="star">' + MULTIPLIER_COMBO + '</div>');
        } else {
          if (i == 1 && suite_horiz == 4) {
            // create a fire gem (can destroy 8 surrounding icons)
            $('.icone[data-ligne=' + ligne + '][data-col=' + (colonne+i) + ']').addClass('fire');
            $('#zone_message').append('<div class="fire">' + MULTIPLIER_FIREBALL + '</div>');
          } else {
            ...
            $('#zone_message').append('<div class="hypercube">' + MULTIPLIER_HYPERCUBE + '</div>');
            } else {
...
function explosion(ligne, colonne) {
  ...
  $('#zone_message').append('<div class="fire">' + GREAT + '</div>');//GREAT!
  setTimeout(function () {
    $explosion.remove();
  }, 600);
